...
const conversationSagas = [
      ...
      try {
        ...
        if (conversationId) {
          ...
          if (conversationMapping[conversationId]) {
            ...
            yield put(setSearchItems({ label: conversationMapping[conversationId].recipientInfo.name, value: conversationMapping[conversationId].recipientInfo._id }));
          ...
          yield put(setSearchItems({ label: conversation.recipientInfo.name, value: conversation.recipientInfo._id }));
  ...
  {
    on: getCountTotalNotRead,
    * worker() {
      try {
        const resp = yield messageService.countTotalNotRead();
        yield put(getCountTotalNotReadSuccess(resp.data));
      } catch (e) {
        // TODO - alert error
      }
    }
  ...
  {
    on: getCountSystemNotRead,
    * worker() {
      try {
        const resp = yield messageService.countSystemNotRead();
        yield put(getCountSystemNotReadSuccess(resp.data));
      } catch (e) {
        // TODO - alert error
      }
    }
...
const messageSagas = [
  ...
  },
  {
    ...
    * worker(req: IReduxAction<any>) {
      try {
        ...
        const resp = yield messageService.sendMessage(data);
  ...
  },
  {
    ...
    * worker(req: IReduxAction<any>) {
      try {
        const { data } = req.payload;
        const resp = yield messageService.sendMediaMessage(data);
        yield put(sendMediaMessageSuccess(resp.data));
      } catch (e) {
        yield put(sendMediaMessageFail(e));
      }
    }
  },
  {
    ...
    * worker(data: IReduxAction<any>) {
      try {
        const resp = yield messageService.getMediaLibraryData(data.payload);
        yield put(getMediasSuccess(resp.data));
      } catch (e) {
        yield put(getMediasFail(e));
      }
    }
  }
];
